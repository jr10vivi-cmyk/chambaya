...
const navItems: NavItem[] = [
  { to: ROUTES.CLIENTE.ROOT, icon: Search, label: "Buscar", end: true },
  { to: ROUTES.CLIENTE.SOLICITUDES, icon: ClipboardList, label: "Solicitudes" },
  { to: ROUTES.CLIENTE.CHAT, icon: MessageCircle, label: "Chat" },
  { to: ROUTES.CLIENTE.HISTORIAL, icon: History, label: "Historial" },
];
        ...
        <nav className="flex-1 p-4 space-y-1">
          {navItems.map(({ to, icon: Icon, label, end }) => (
            ...
            >
              <Icon size={18} />
              {label}
              {to === ROUTES.CLIENTE.CHAT && noLeidos > 0 && (
