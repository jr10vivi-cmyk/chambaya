...
const navItems: NavItem[] = [
  {
    to: ROUTES.TECNICO.ROOT,
    icon: LayoutDashboard,
    label: "Inicio",
    end: true,
  },
  { to: ROUTES.TECNICO.SOLICITUDES, icon: ClipboardList, label: "Solicitudes" },
  { to: ROUTES.TECNICO.TRABAJOS, icon: Briefcase, label: "Trabajos" },
  { to: ROUTES.TECNICO.GANANCIAS, icon: DollarSign, label: "Ganancias" },
  { to: ROUTES.TECNICO.PLANES, icon: Crown, label: "Premium" },
  { to: ROUTES.TECNICO.CHAT, icon: MessageCircle, label: "Chat" },
  { to: ROUTES.TECNICO.PERFIL, icon: User, label: "Mi Perfil" },
];
        ...
        <nav className="flex-1 p-4 space-y-1">
          {navItems.map(({ to, icon: Icon, label, end }) => (
            ...
            >
              <Icon size={18} />
              {label}
              {to === ROUTES.TECNICO.CHAT && noLeidos > 0 && (
