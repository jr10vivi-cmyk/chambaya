 ...
 */
export default function BannerPublicidad({ posicion = 'inicio', tipo, className = '' }: BannerPublicidadProps) {
  ...
  useEffect(() => {
    ...
    q.limit(5).then(({ data }) => {
      ...
      lista.forEach(a => {
        supabase.rpc('registrar_impresion', { p_anuncio_id: a.id }).then(() => {})
      })
    })
  }, [posicion, tipo])
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => setIndice(i => (i + 1) % anuncios.length), 6000)
    return () => clearInterval(t)
  }, [anuncios.length])

  const handleClick = (anuncio: Anuncio) => {
    supabase.rpc('registrar_click', { p_anuncio_id: anuncio.id }).then(() => {})
    if (anuncio.url_destino) window.open(anuncio.url_destino, '_blank', 'noopener')
  }
  ...
  return (
    <div className={`relative overflow-hidden rounded-2xl ${className}`}>
      ...
      >
        ...
        {/* Boton cerrar */}
        <button
          onClick={(e: React.MouseEvent<HTMLButtonElement>) => { e.stopPropagation(); setCerrados(s => new Set([...s, anuncio.id])) }}
        ...
        <div className="mt-4 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-bold text-white text-sm leading-tight">{anuncio.titulo}</p>
            {anuncio.descripcion && (
              <p className="text-white/80 text-xs mt-0.5 line-clamp-1">{anuncio.descripcion}</p>
            )}
            {anuncio.url_destino && (
        ...
        {visibles.length > 1 && (
          <div className="flex justify-center gap-1 mt-3">
            {visibles.map((_, i) => (
              <button
                key={i}
                onClick={(e: React.MouseEvent<HTMLButtonElement>) => { e.stopPropagation(); setIndice(i) }}
