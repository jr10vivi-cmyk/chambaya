...
import {
    ...
    Star, MapPin, Clock, Crown, ChevronDown,
    RefreshCw, Ban, MoreVertical
} from 'lucide-react'
...
const ESTADO_CFG = {
    ...
    suspendido: { color: 'bg-gray-100 text-gray-600 border-gray-200', label: 'Suspendido' },
}

// ── Sub-componentes ──────────────────────────────────────────

function EstadoBadge({ estado }) {
    const cfg = ESTADO_CFG[estado] || { color: 'bg-gray-100 text-gray-600 border-gray-200', label: estado }
    return (
...
function AccionesMenu({ tecnico, onAction }) {
    ...
    const acciones = [
        ...
        { label: 'Aprobar', action: 'aprobar', icon: CheckCircle, show: tecnico.estado_verificacion !== 'aprobado' },
        { label: 'Rechazar', action: 'rechazar', icon: XCircle, show: tecnico.estado_verificacion !== 'rechazado' },
        { label: 'Suspender', action: 'suspender', icon: Ban, show: tecnico.estado_verificacion !== 'suspendido' },
        { label: 'Dar Premium', action: 'premium', icon: Crown, show: !tecnico.es_premium },
        { label: 'Quitar Premium', action: 'sin_premium', icon: Crown, show: tecnico.es_premium },
                ...
                <>
                    ...
                    <div className="absolute right-0 top-8 bg-white border border-gray-200 rounded-xl shadow-lg z-20 py-1 w-44">
                        {acciones.map(({ label, action, icon: Icon }) => (
                            <button
                                key={action}
                                onClick={() => { onAction(action, tecnico); setOpen(false) }}
            ...
            <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
                ...
                <div className="p-6 space-y-5">
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        {[
                            { label: 'Calificación', value: tecnico.calificacion_promedio?.toFixed(1) || '—', icon: Star },
                            { label: 'Trabajos', value: tecnico.total_trabajos || 0, icon: CheckCircle },
                            { label: 'Tarifa/hr', value: tecnico.tarifa_hora ? `S/${tecnico.tarifa_hora}` : '—', icon: Clock },
                    ...
                    <div className="space-y-2 text-sm">
                        {[
                            ...
                            ['Experiencia', tecnico.experiencia_anos ? `${tecnico.experiencia_anos} año(s)` : 'No especificada'],
                            ['Radio servicio', tecnico.radio_servicio_km ? `${tecnico.radio_servicio_km} km` : 'No especificado'],
                        ].map(([k, v]) => (
                            <div key={k} className="flex gap-2">
                                <span className="font-medium text-gray-500 w-28 flex-shrink-0">{k}:</span>
                    ...
                    {tecnico.tecnico_categorias?.length > 0 && (
                        <div>
                            <p className="text-sm font-medium text-gray-500 mb-2">Categorías de servicio:</p>
                            <div className="flex flex-wrap gap-2">
                                {tecnico.tecnico_categorias.map(tc => (
                                    <span key={tc.categoria_id} className="px-2.5 py-1 bg-orange-50 text-orange-700 rounded-lg text-xs font-medium">
                    ...
                    <p className="text-xs text-gray-400">
                        Registrado el {new Date(tecnico.creado_en).toLocaleDateString('es-PE', {
...
export default function AdminTecnicos() {
    ...
    const handleAccion = async (accion, tecnico) => {
        if (accion === 'ver') { setTecnicoDetalle(tecnico); return }
        ...
        try {
            ...
            if (accion === 'aprobar' || accion === 'rechazar') {
                await supabase.from('notificaciones').insert({
                    ...
                    tipo: 'verificacion',
                    titulo: accion === 'aprobar' ? '¡Tu cuenta fue aprobada!' : 'Tu cuenta fue rechazada',
    ...
    const conteos = {
        pendiente: tecnicos.filter(t => t.estado_verificacion === 'pendiente').length,
        aprobado: tecnicos.filter(t => t.estado_verificacion === 'aprobado').length,
        ...
        <div className="max-w-7xl space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">{tecnicos.length} técnico(s) encontrado(s)</p>
                </div>
                <button onClick={fetchTecnicos}
            ...
            <div className="grid grid-cols-3 gap-4">
                {[
                    { label: 'Pendientes', value: conteos.pendiente, color: 'text-amber-600 bg-amber-50 border-amber-200' },
                    { label: 'Aprobados', value: conteos.aprobado, color: 'text-green-600 bg-green-50 border-green-200' },
                    { label: 'Premium', value: conteos.premium, color: 'text-purple-600 bg-purple-50 border-purple-200' },
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="border-b border-gray-100 bg-gray-50">
                                    {['Técnico', 'Ciudad', 'Categorías', 'Calificación', 'Trabajos', 'Estado', ''].map(h => (
                                    ...
                                    <tr key={t.id}
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="text-sm font-semibold text-gray-800 flex items-center gap-1.5">
                                                        ...
                                                        {t.es_premium && <Crown size={12} className="text-amber-500" fill="currentColor" />}
                                                    </p>
                                        ...
                                        <td className="px-5 py-4">
                                            <div className="flex flex-wrap gap-1 max-w-[160px]">
                                                ...
                                                {!t.tecnico_categorias?.length && <span className="text-xs text-gray-300">Sin categoría</span>}
                                            </div>
                                        </td>
