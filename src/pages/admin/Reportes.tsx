...
import {
  ...
  Wrench, Download, Star, CheckCircle2, Clock,
...
function GraficoBarras({ datos }: { datos: { fecha: string; total: number }[] }) {
...
function KpiCard({ icon: Icon, label, value, sub, color, bgColor }: KpiCardProps) {
  return (
...
const ESTADO_CFG = {
  completado:  { label: 'Completados',  color: '#22c55e', bg: 'bg-green-50',  text: 'text-green-700',  icon: CheckCircle2 },
  en_proceso:  { label: 'En proceso',   color: '#8b5cf6', bg: 'bg-purple-50', text: 'text-purple-700', icon: Loader2 },
  aceptado:    { label: 'Aceptados',    color: '#3b82f6', bg: 'bg-blue-50',   text: 'text-blue-700',   icon: ShoppingBag },
  pendiente:   { label: 'Pendientes',   color: '#f59e0b', bg: 'bg-amber-50',  text: 'text-amber-700',  icon: Clock },
  cancelado:   { label: 'Cancelados',   color: '#ef4444', bg: 'bg-red-50',    text: 'text-red-700',    icon: XCircle },
} as const
...
export default function AdminReportes() {
  ...
  useEffect(() => { fetchReportes(rango) }, [rango, fetchReportes])
  ...
  const segmentosSolicitudes = (Object.keys(ESTADO_CFG) as EstadoKey[]).map(k => ({
    ...
    <div className="space-y-6 max-w-7xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">Análisis de actividad y rendimiento de la plataforma</p>
        </div>
      ...
      <div className="bg-white rounded-2xl p-6 border border-gray-100">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="ml-auto text-sm text-gray-400">últimos {rango} días</span>
        </div>
        {d.ingresoPorDia.every(x => x.total === 0)
          ? <p className="text-center text-gray-400 text-sm py-10">Aún no hay ingresos registrados en este período</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* Ingresos por tipo */}
        <div className="bg-white rounded-2xl p-6 border border-gray-100">
          <h3 className="font-semibold text-gray-800 mb-5">Composición de ingresos</h3>
          <div className="flex items-center gap-6">
            ...
            <div className="flex-1 space-y-3">
              {[
                { label: 'Comisiones (10%)', valor: d.totales.comision,    color: 'bg-orange-500' },
                { label: 'Publicidad',        valor: d.totales.publicidad,  color: 'bg-blue-500' },
                { label: 'Suscripciones',     valor: d.totales.suscripcion, color: 'bg-purple-500' },
              ].map(({ label, valor, color }) => {
                const pct = d.totales.total > 0 ? ((valor / d.totales.total) * 100).toFixed(0) : 0
                return (
                  <div key={label}>
        ...
        <div className="bg-white rounded-2xl p-6 border border-gray-100">
          <h3 className="font-semibold text-gray-800 mb-5">Solicitudes por estado</h3>
          {d.totalSolicitudes === 0
            ? <p className="text-center text-gray-400 text-sm py-10">Sin solicitudes en este período</p>
            : (
              <div className="flex items-center gap-6">
                <GraficoDonut
                ...
                <div className="flex-1 space-y-2">
                  {(Object.keys(ESTADO_CFG) as EstadoKey[]).map(k => {
                    ...
                    const pct = d.totalSolicitudes > 0 ? ((cant / d.totalSolicitudes) * 100).toFixed(0) : 0
                    return (
                      <div key={k}>
                        <div className="flex justify-between text-sm mb-1">
                          ...
                          <span className="font-medium text-gray-700">{cant} <span className="text-gray-400 font-normal">({pct}%)</span></span>
                        </div>
                        <div className="w-full bg-gray-100 rounded-full h-1">
                          <div className="h-1 rounded-full" style={{ width: `${pct}%`, background: cfg.color }} />
                        </div>
                      </div>
                    )
                  })}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* Top técnicos */}
        <div className="bg-white rounded-2xl p-6 border border-gray-100">
          <h3 className="font-semibold text-gray-800 mb-5">Top técnicos por trabajos</h3>
                ...
                {d.tecnicosTop.slice(0, 8).map((t, i) => (
                  <div key={t.id} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-400">{t.profiles?.ciudad} · {t.total_trabajos} trabajos · {t.total_resenas} reseñas</p>
                    </div>
        ...
        <div className="bg-white rounded-2xl p-6 border border-gray-100">
          <h3 className="font-semibold text-gray-800 mb-5">Categorías más demandadas</h3>
            ...
            : (
              <div className="space-y-3">
                {d.categoriasPopulares.map((cat, i) => {
                  ...
                  const colores = ['bg-orange-500', 'bg-blue-500', 'bg-green-500', 'bg-purple-500', 'bg-amber-500', 'bg-red-500']
                  return (
                    <div key={cat.nombre}>
                      <div className="flex justify-between text-sm mb-1.5">
                        <span className="text-gray-700 font-medium">{cat.nombre}</span>
                        <span className="text-gray-400">{cat.count} técnicos</span>
                      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
          <h3 className="font-semibold text-gray-800">Últimos ingresos registrados</h3>
          <button
            onClick={handleExport}
        ...
        {d.ingresosDetalle.length === 0
          ? <p className="text-center text-gray-400 text-sm py-10">Sin ingresos registrados</p>
          : (
                ...
                <tbody className="divide-y divide-gray-50">
                  {d.ingresosDetalle.map((ing, i) => {
                    const tipoCfg: Record<string, { label: string; color: string }> = {
                      comision:    { label: 'Comisión',     color: 'bg-orange-50 text-orange-700' },
                      publicidad:  { label: 'Publicidad',   color: 'bg-blue-50 text-blue-700' },
                      suscripcion: { label: 'Suscripción',  color: 'bg-purple-50 text-purple-700' },
                    }
                    const cfg = tipoCfg[ing.tipo ?? ''] ?? { label: ing.tipo ?? '—', color: 'bg-gray-100 text-gray-600' }
                    return (
