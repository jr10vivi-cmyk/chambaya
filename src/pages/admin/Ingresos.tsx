    ...
    <div className="max-w-5xl space-y-6">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Comisiones (10%), publicidad y suscripciones
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <KpiCard
          icon={Percent}
          label="Comisiones (10%)"
          value={fmt(kpis.comision)}
          color="orange"
        />
        <KpiCard
          icon={TrendingUp}
          label="Publicidad"
          value={fmt(kpis.publicidad)}
          color="blue"
        />
        <KpiCard
          icon={CreditCard}
          label="Suscripciones"
          value={fmt(kpis.suscripcion)}
          color="purple"
        />
      </div>
      ...
      {kpis.total > 0 && (
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          ...
          <div className="flex rounded-full h-4">
            {[
              {
                pct: (kpis.comision / kpis.total) * 100,
                color: "bg-orange-500",
              },
              {
                pct: (kpis.publicidad / kpis.total) * 100,
                color: "bg-blue-500",
              },
              {
                pct: (kpis.suscripcion / kpis.total) * 100,
                color: "bg-purple-500",
              },
            ].map(
              (seg, i) =>
            ...
            {[
              {
                ...
                value:
                  kpis.total > 0
                    ? ((kpis.comision / kpis.total) * 100).toFixed(0)
                    : 0,
              },
              {
                color: "bg-blue-500",
                ...
                value:
                  kpis.total > 0
                    ? ((kpis.publicidad / kpis.total) * 100).toFixed(0)
                    : 0,
              },
              {
                color: "bg-purple-500",
                ...
                value:
                  kpis.total > 0
                    ? ((kpis.suscripcion / kpis.total) * 100).toFixed(0)
                    : 0,
              },
            ].map((seg) => (
              <div
            ...
            <tbody className="divide-y divide-gray-50">
              ...
              {ingresos.map((ing, i) => {
                const cfg = tipoCfg[ing.tipo] || {
                  color: "bg-gray-100 text-gray-600 border-gray-100",
                  label: ing.tipo,
                };
                return (
