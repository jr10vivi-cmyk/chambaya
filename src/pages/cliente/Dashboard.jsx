...
import {
  Search, ClipboardList, MessageCircle, Star,
  Crown, Zap, MapPin, ChevronRight, Clock, CheckCircle
} from 'lucide-react'
...
function TecnicoDestacadoCard({ tecnico }) {
  ...
  return (
    <Link to={`/cliente/tecnico/${tecnico.id}`}
      ...
      <div className="p-4">
        <div className="flex items-center gap-3 mb-3">
          <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-bold text-lg flex-shrink-0
            ...
            {tecnico.profiles?.avatar_url
              ? <img src={tecnico.profiles.avatar_url} alt={nombre} className="w-full h-full object-cover rounded-xl" />
        ...
        <div className="flex items-center justify-between text-xs">
          <div className="flex items-center gap-1">
            ...
            <span className="text-gray-400">({tecnico.total_resenas || 0})</span>
          </div>
          {tecnico.tarifa_hora && (
            <span className="font-semibold text-orange-600">S/{tecnico.tarifa_hora}/h</span>
          )}
        </div>
...
export default function ClienteDashboard() {
  ...
  const [stats, setStats] = useState({ pendientes: 0, completados: 0, gastado: 0 })
  ...
  useEffect(() => {
    ...
    const cargarDatos = async () => {
      ...
      const arr = sols || []
      const pendientes = arr.filter(s => ['pendiente', 'aceptado', 'en_proceso'].includes(s.estado)).length
      ...
      const gastado = (pagos || []).reduce((s, p) => s + Number(p.monto_total), 0)
  ...
  return (
    <div className="max-w-4xl space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { to: '/cliente/buscar', icon: Search, label: 'Buscar tecnico', color: 'bg-orange-50 text-orange-600' },
          { to: '/cliente/solicitudes', icon: ClipboardList, label: 'Mis solicitudes', color: 'bg-blue-50 text-blue-600', badge: stats.pendientes },
          { to: '/cliente/chat', icon: MessageCircle, label: 'Chat', color: 'bg-green-50 text-green-600' },
          { to: '/cliente/historial', icon: Clock, label: 'Historial', color: 'bg-purple-50 text-purple-600' },
      ...
      {!loading && (
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-4 text-center">
            ...
            <p className="text-lg font-bold text-gray-900">{fmt(stats.gastado)}</p>
      ...
      <div>
        ...
        {loading ? (
          ...
          <div className="text-center py-10 bg-white rounded-2xl border border-gray-100">
            <p className="text-gray-400 text-sm">Sin tecnicos disponibles aun</p>
          </div>
        ) : (
