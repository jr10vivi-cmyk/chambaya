...
import {
  User, MapPin, DollarSign, Briefcase, Tag,
  Camera, Save, Crown, Star, CheckCircle,
  AlertCircle, Shield, LocateFixed, Loader2
} from 'lucide-react'
...
const INPUT = "w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-transparent transition"
...
export default function TecnicoPerfil() {
  ...
  const [prof, setProf] = useState({
    ...
    tarifa_hora: '', tarifa_minima: '',
    radio_servicio_km: 10, disponible: true,
    lat: null, lng: null
  })
  ...
  useEffect(() => {
    ...
    if (profile?.id) {
      supabase.from('tecnico_categorias').select('categoria_id').eq('tecnico_id', profile.id)
        .then(({ data }) => setCatsSelec(data?.map(c => c.categoria_id) || []))
    }
  }, [profile, tecnico])
  ...
  const handleGPS = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      pos => {
        setProf(p => ({ ...p, lat: pos.coords.latitude, lng: pos.coords.longitude }))
        setLocLoading(false)
        toast.success('Ubicación obtenida ✓')
      },
      () => { toast.error('No se pudo obtener la ubicación'); setLocLoading(false) }
    )
  }
  ...
  const handleGuardar = async (e) => {
    ...
    if (catsSelec.length === 0)  { toast.error('Selecciona al menos una categoría'); return }
    setGuardando(true)
    ...
    try {
      ...
      await supabase.from('tecnico_categorias').delete().eq('tecnico_id', profile.id)
      if (catsSelec.length > 0) {
        await supabase.from('tecnico_categorias').insert(
          catsSelec.map(cid => ({ tecnico_id: profile.id, categoria_id: cid }))
        )
      }
  ...
  return (
    <form onSubmit={handleGuardar} className="max-w-2xl space-y-6">
      ...
      {tecnico && (
        ...
        >
          {{
            aprobado:   <><Shield size={16} className="text-green-500" /> Cuenta verificada y activa</>,
            pendiente:  <><AlertCircle size={16} className="text-amber-500" /> En revisión por el equipo ChambaYA</>,
            rechazado:  <><AlertCircle size={16} className="text-red-500" /> Cuenta no aprobada — contacta soporte</>,
            suspendido: <><AlertCircle size={16} /> Cuenta suspendida</>,
          }[tecnico.estado_verificacion]}
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6">
        <div className="flex items-center gap-5">
          <div className="relative">
            <div className="w-20 h-20 rounded-2xl bg-orange-100 flex items-center justify-center overflow-hidden">
              {avatarUrl
                ? <img src={avatarUrl} alt="Avatar" className="w-full h-full object-cover" />
                : <span className="text-3xl font-bold text-orange-600">{inicial}</span>
              }
            </div>
            <button
              type="button"
          ...
          <div>
            <p className="font-semibold text-gray-900">{personal.nombre} {personal.apellido}</p>
      ...
      {/* ── Datos personales ── */}
      <Seccion titulo="Datos personales" icono={User} color="text-blue-600 bg-blue-50">
        <div className="grid grid-cols-2 gap-4">
          <Campo label="Nombre *">
            <input required value={personal.nombre}
              onChange={e => setPersonal(p => ({ ...p, nombre: e.target.value }))}
              className={INPUT} placeholder="Tu nombre" />
          </Campo>
          <Campo label="Apellido">
            <input value={personal.apellido}
              onChange={e => setPersonal(p => ({ ...p, apellido: e.target.value }))}
              className={INPUT} placeholder="Tu apellido" />
          </Campo>
        </div>
        <div className="grid grid-cols-2 gap-4 mt-4">
          <Campo label="Teléfono" hint="Oculto para clientes por seguridad">
            <input type="tel" value={personal.telefono}
              onChange={e => setPersonal(p => ({ ...p, telefono: e.target.value }))}
              className={INPUT} placeholder="999 999 999" />
          </Campo>
          <Campo label="Ciudad">
            <input value={personal.ciudad}
              onChange={e => setPersonal(p => ({ ...p, ciudad: e.target.value }))}
              className={INPUT} placeholder="Ayacucho" />
          </Campo>
        </div>
        <div className="mt-4">
          <Campo label="Departamento">
            <select value={personal.departamento}
              onChange={e => setPersonal(p => ({ ...p, departamento: e.target.value }))}
              className={INPUT}>
      ...
      {/* ── Datos profesionales ── */}
      <Seccion titulo="Información profesional" icono={Briefcase} color="text-orange-600 bg-orange-50">
        <div className="space-y-4">
          <Campo label="Descripción de tus servicios *"
            hint="Cuéntale a los clientes qué haces y tu experiencia. Mín. 50 caracteres.">
            <textarea value={prof.descripcion} rows={4}
              onChange={e => setProf(p => ({ ...p, descripcion: e.target.value }))}
          ...
          <div className="grid grid-cols-3 gap-4">
            <Campo label="Años de experiencia">
              <input type="number" min={0} max={50}
                ...
                onChange={e => setProf(p => ({ ...p, experiencia_anos: e.target.value }))}
                className={INPUT} />
            </Campo>
            <Campo label="Tarifa por hora (S/)">
              <input type="number" min={0} step="0.50"
                value={prof.tarifa_hora}
                onChange={e => setProf(p => ({ ...p, tarifa_hora: e.target.value }))}
                className={INPUT} placeholder="35.00" />
            </Campo>
            <Campo label="Tarifa mínima (S/)">
              <input type="number" min={0} step="0.50"
                value={prof.tarifa_minima}
                onChange={e => setProf(p => ({ ...p, tarifa_minima: e.target.value }))}
                className={INPUT} placeholder="50.00" />
            </Campo>
          </div>

          <div className="grid grid-cols-2 gap-4">
            <Campo label="Radio de servicio (km)">
              <select value={prof.radio_servicio_km}
                onChange={e => setProf(p => ({ ...p, radio_servicio_km: e.target.value }))}
                className={INPUT}>
                {[2, 5, 10, 20, 50].map(r => (
            ...
            <Campo label="Disponibilidad">
              <div
                onClick={() => setProf(p => ({ ...p, disponible: !p.disponible }))}
              ...
              >
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0
                  ...
                  {prof.disponible && <CheckCircle size={12} className="text-white" />}
                </div>
                <div>
                  <p className={`text-sm font-semibold ${prof.disponible ? 'text-green-700' : 'text-gray-500'}`}>
                    {prof.disponible ? 'Disponible' : 'No disponible'}
                  </p>
                  <p className="text-xs text-gray-400">Los clientes pueden encontrarte</p>
                </div>
              </div>
            </Campo>
          </div>
      ...
      {/* ── Categorías ── */}
      <Seccion titulo="Categorías de servicio *" icono={Tag} color="text-purple-600 bg-purple-50">
        <p className="text-xs text-gray-400 mb-4">
          Selecciona todos los oficios que ofreces. Aparecerás en los resultados de búsqueda de cada categoría.
        </p>
        ...
        {catsSelec.length > 0 && (
          <p className="text-xs text-orange-600 font-medium mt-3">
            ✓ {catsSelec.length} categoría{catsSelec.length > 1 ? 's' : ''} seleccionada{catsSelec.length > 1 ? 's' : ''}
          </p>
        )}
      </Seccion>

      {/* ── Ubicación ── */}
      <Seccion titulo="Tu ubicación en el mapa" icono={MapPin} color="text-teal-600 bg-teal-50">
        <p className="text-xs text-gray-400 mb-4">
          Los clientes te encontrarán en el mapa. Haz clic en el mapa o usa tu GPS para marcar tu zona de trabajo.
        </p>
        ...
        <p className="text-xs text-gray-400 mt-2 flex items-center gap-1">
          <Shield size={11} />
          Tu ubicación exacta no es visible para los clientes, solo tu zona aproximada.
        </p>
      </Seccion>

      {/* ── Suscripción Premium ── */}
      {!tecnico?.es_premium ? (
        <Seccion titulo="Plan Premium" icono={Crown} color="text-amber-600 bg-amber-50">
          <p className="text-sm text-gray-600 mb-5">
            Destácate sobre la competencia y recibe más clientes con el plan Premium.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              { plan: 'mensual',     precio: 29.90,  label: '1 mes',    ahorro: null    },
              { plan: 'trimestral',  precio: 79.90,  label: '3 meses',  ahorro: '11%'   },
              { plan: 'anual',       precio: 249.90, label: '12 meses', ahorro: '30%'   },
            ].map(({ plan, precio, label, ahorro }) => (
              <div key={plan} className="border-2 border-gray-200 hover:border-amber-400 rounded-xl p-4 text-center cursor-pointer transition group">
                {ahorro && (
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-white/80">
                Activo hasta {new Date(tecnico.premium_hasta).toLocaleDateString('es-PE', {
