...
import {
  TrendingUp, DollarSign, Briefcase, Percent,
      ...
      {data.map((d, i) => (
        <div key={i} className="flex-1 flex flex-col items-center gap-1">
          <div
            ...
            style={{ height: `${Math.max((d.monto / max) * 52, d.monto > 0 ? 6 : 2)}px` }}
          />
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">El técnico recibe el 90% por cada trabajo completado</p>
        </div>

        {/* Saldo disponible */}
        {saldo && (
          <div className="bg-gradient-to-br from-orange-500 to-orange-600 rounded-2xl p-4 text-white min-w-[180px]">
            <p className="text-xs font-medium opacity-80 mb-1">Saldo disponible</p>
            <p className="text-2xl font-bold">{fmt(saldo.saldo_disponible)}</p>
            <p className="text-xs opacity-70 mt-1">Total histórico: {fmt(saldo.saldo_total)}</p>
          </div>
        )}
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="font-semibold text-gray-800">Ganancias últimos 7 días</h3>
        </div>
        {kpis?.graficoDias?.every(d => d.monto === 0)
          ? <p className="text-sm text-gray-400 text-center py-6">Sin ganancias en este período</p>
      ...
      <div className="bg-amber-50 border border-amber-100 rounded-2xl p-4 flex items-start gap-3">
        ...
        <div>
          <p className="text-sm font-semibold text-amber-800">¿Cómo funciona el pago?</p>
          <p className="text-xs text-amber-700 mt-1 leading-relaxed">
            Cuando el cliente confirma que el trabajo fue completado, ChambaYA retiene el <strong>10%</strong> como
            comisión de plataforma y el restante <strong>90%</strong> se acredita a tu saldo disponible.
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        <div className="p-5 border-b border-gray-50">
          ...
          <p className="text-xs text-gray-400 mt-0.5">{pagos.length} registro{pagos.length !== 1 ? 's' : ''}</p>
        </div>

        {pagos.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-14 text-center">
            <DollarSign size={36} className="text-gray-200 mb-3" />
            <p className="font-medium text-gray-500">Sin pagos en este período</p>
            <p className="text-sm text-gray-400 mt-1">Completa trabajos para ver tus ganancias aquí</p>
          </div>
        ) : (
          <div className="divide-y divide-gray-50">
            {pagos.map(pago => {
              const clienteNombre = `${pago.profiles?.nombre || ''} ${pago.profiles?.apellido || ''}`.trim()
              return (
                <div key={pago.id} className="px-5 py-4 hover:bg-gray-50 transition">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="text-right flex-shrink-0">
                      <p className="text-sm font-bold text-green-600">{fmt(pago.monto_tecnico)}</p>
                      <p className="text-xs text-gray-400">
                        de {fmt(pago.monto_total)} · comisión {fmt(pago.comision)}
                      </p>
                    </div>
                  </div>
